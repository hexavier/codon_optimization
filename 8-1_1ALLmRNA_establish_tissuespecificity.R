...
## Load PTR data
# GTEX data
gtex = read.csv("data/mRNAseq_medians_GTEx_Jiang2020.csv", row.names = 1)
# Eraslan(2019) data
eraslan = read.csv("data/mRNAseq_FPKM_Eraslan2019.csv", row.names = 2)
...
# Save
write.table(TSmergedprots,sprintf("results/TSall_mRNA-UP-FCavg%s-FCglob%s.tsv",nfold_avg,nfold_glob),sep="\t",row.names = F)
...
write.csv(overlap_df,sprintf("results/mRNA_overlap_datasets_TSproteins-FCavg%s-FCglob%s.csv",nfold_avg,nfold_glob))
