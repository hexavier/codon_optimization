...
dinucl = pd.read_csv("data/human_CU_ENS_dinucl.tsv", sep="\t", index_col=0)
...
for t in tissues:
    ...
    genes = [s for s in codus.index if (s in TStemp.index) and (s in dinucl.index)]
    codusnorm = codus.loc[genes,codons].apply(norm_len, axis=1)
    dinuclnorm = dinucl.loc[genes,DINUCL].apply(norm_len, axis=1)
    # Create dataset
    indf = pd.concat([codusnorm,dinuclnorm,TStemp.loc[genes,["updown"]]], axis=1)
...
#%% #ROC curve
    ...
    # show the plot
    plt.show()
    fig.savefig(str("plots/all_RandomForest_codus+dinucl_to_UPDOWNproteins/roc_proteins_%s.pdf" % (t)), bbox_inches='tight')
    
#%% Save
RFmeans.to_csv("results/mean_all_randomforest_codus+dinucl_to_UPDOWNproteins.csv")
RFstd.to_csv("results/std_all_randomforest_codus+dinucl_to_UPDOWNproteins.csv")
