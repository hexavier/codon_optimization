...
## Load PTR data
# Eraslan(2019) data
tissuePTR = read.csv("data/PTR_Eraslan2019.tsv", sep="\t", row.names = 2)
tissuePTR = tissuePTR[,4:ncol(tissuePTR)]
...
# Save
write.table(TSmergedprots,sprintf("results/TSeraslan_PTR-UP-FCavg%s-FCglob%s.tsv",nfold_avg,nfold_glob),sep="\t",row.names = F)
